...
PYF(pgtick)
{
    ...
    float v = 0.0, tikl = 0.0, tikr = 0.0, disp = 1.0, orient = 0.0;
    char *text = "ppgpot";

    if (!PyArg_ParseTuple(args,"fffffffffs:pgtick", &x1, &y1, &x2, &y2, &v, &tikl, &tikr, &disp, &orient, &text))
    return(NULL);

    cpgtick(x1, y1, x2, y2, v, tikl, tikr, disp, orient, text);
